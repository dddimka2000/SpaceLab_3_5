...
public class BasketController {
    ...
    public String showWishList(@AuthenticationPrincipal UserDetailsImpl userDetails, Model model) {
        ...
        model.addAttribute("count", basketItemService.findAllBasketProductsByUserEntity(userDetails.getUserEntity())
                .stream().map(s -> s.getCount()).toList());
        model.addAttribute("products", basketItemService.findAllBasketProductsByUserEntity(userDetails.getUserEntity()).stream().map(s -> s.getProductEntity()).toList());
